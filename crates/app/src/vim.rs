...
pub fn ejecutar_tecla_normal(c: char, layout: &mut PanelLayout, vim: &mut EstadoVim, config: &Config) {
    ...
    let mensaje = aviso.or_else(|| (!pendientes.is_empty()).then_some(pendientes));
    if mensaje.is_some() {
